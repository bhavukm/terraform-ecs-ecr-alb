...
ALB + target group + listener: routes HTTP traffic to tasks, health checks ensure only healthy tasks receive traffic.
IAM execution role + Secrets Manager mapping: lets tasks securely retrieve secrets (no plaintext creds in code).
CloudWatch logs: centralizes application logs for troubleshooting.
